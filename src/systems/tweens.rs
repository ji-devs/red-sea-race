...
pub fn run(
    ...
    {
        (&tween_event_storage, &tweens_lookup_storage)
            .iter()
            .with_id()
            .for_each(|(entity, (event, tweens_lookup))| {
                entities_with_events.push(entity);
                if let Some((timeline, ending)) = match event {
                    TweenEvent::StartByName(name, ending) => {
                        let name:&'static str = name;
                        let timeline = tweens_lookup.0.get(name).unwrap_throw();
    ...
    (&mut tween_player_storage)
        ...
        .for_each(|player| {
            ...
            if player.playhead <= player.duration {
                if let Some(active_tweens) = player.timeline.get_active_tweens(player.playhead) {
                    for (progress, tween) in active_tweens {
                        let entity = tween.info().entity;
